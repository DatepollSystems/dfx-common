...
export class UIHelper {
  ...
  public static getApproxCurrentDate(): Date {
   ...
   * @param {Date} toDate Start date (default getApproxCurrentDate())
   * @return string
   */
  public static getTimeLeft(date: Date, toDate: NullOr<Date> = null): string {
    if (!toDate) {
      toDate = this.getApproxCurrentDate();
    }

    const days = Math.round((date.getTime() - toDate.getTime()) / (1000 * 60 * 60 * 24));

    if (days === 0) {
      const hours = Math.round(Math.abs(date.getTime() - toDate.getTime()) / (60 * 60 * 1000));
      return hours + 'h';
    } else if (days > 31) {
      const months = date.getFullYear() * 12 + date.getMonth() - (toDate.getFullYear() * 12 + toDate.getMonth());
      return months + 'mo';
    } else {
      return days + 'd';
    }
